...
import { createTheme, logWarning, monacoLanguageRegistry, SelectableValue } from '@grafana/data';
...
export const Page = () => {
  ...
  useAsync(async () => {
    ...
    if (!response.ok) {
      logWarning('No user found, show login button');
      return;
    }
    const val = await response.json();
    setUserView({
