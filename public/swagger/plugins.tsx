...
const logger = createStructuredLogger('swagger.plugins');
    ...
    wrapComponents: {
      ...
      RequestBody: (Original: React.ElementType) => (props: UntypedProps) => {
        ...
        if (content) {
          ...
          logger.logDebug('Swagger request body schema resolved', { schema: v });
        }
        return (
      ...
      modelExample: (Original: React.ElementType) => (props: UntypedProps) => {
        if (props.isExecute && props.schema) {
          logger.logDebug('Swagger model example resolved', { hasSchema: Boolean(props.schema) });
          return (
          ...
          <SchemaContext.Consumer>
            {(schema) => {
              if (schema) {
                ...
                logger.logDebug('Swagger CodeEditor schema applied', { schema });
