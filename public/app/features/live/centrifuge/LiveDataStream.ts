...
const logger = createMonitoringLogger('features.live.centrifuge');
...
export class LiveDataStream<T = unknown> {
  ...
  private onError = (err: unknown) => {
    logger.logError(err instanceof Error ? err : new Error(String(err)), { channelId: this.deps.channelId });
    this.stream.next({
  ...
  private onComplete = () => {
    logger.logDebug('LiveQuery completed', { channelId: this.deps.channelId });
    this.shutdown();
  };
