...
const log = createStructuredLogger('public.app.features.live.centrifuge.LiveDataStream');
...
export class LiveDataStream<T = unknown> {
  ...
  private onError = (err: unknown) => {
    log.debug('Live query stream error', { channelId: this.deps.channelId, error: err instanceof Error ? err.message : String(err) });
    this.stream.next({
  ...
  private onComplete = () => {
    log.debug('Live query stream completed', { channelId: this.deps.channelId });
    this.shutdown();
  };
