...
import { liveLogger } from '../utils/logging';
...
export class LiveDataStream<T = unknown> {
  ...
  private onError = (err: unknown) => {
    liveLogger.logError(err instanceof Error ? err : new Error(String(err)), { channelId: this.deps.channelId, context: 'LiveQuery error' });
    this.stream.next({
  ...
  private onComplete = () => {
    liveLogger.logDebug('LiveQuery complete', { channelId: this.deps.channelId });
    this.shutdown();
  };
