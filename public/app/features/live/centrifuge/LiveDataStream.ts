...
const logger = createMonitoringLogger('live.centrifuge.data-stream');
...
export class LiveDataStream<T = unknown> {
  ...
  private onError = (err: unknown) => {
    logger.logWarning('Live query error', { error: String(err), channelId: this.deps.channelId });
    this.stream.next({
  ...
  private onComplete = () => {
    logger.logDebug('Live query complete', { channelId: this.deps.channelId });
    this.shutdown();
  };
