import { logWarning, PluginError, PluginMeta, renderMarkdown } from '@grafana/data';
...
export async function getRemotePlugins(): Promise<RemotePlugin[]> {
  try {
    ...
    if (isFetchError(error)) {
      ...
      error.isHandled = true;
      logWarning('Failed to fetch plugins from catalog (default https://grafana.com/api/plugins)');
      return [];
    }
