...
const logger = createMonitoringLogger('features.plugins.admin.actions');
  ...
  try {
    ...
    forkJoin({
      ...
      .subscribe(
        ...
        (error) => {
          logger.logError(error instanceof Error ? error : new Error(String(error)), { context: 'fetchAll' });
