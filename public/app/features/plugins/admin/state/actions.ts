...
const log = createStructuredLogger('public.app.features.plugins.admin.state.actions');
  ...
  try {
    ...
    forkJoin({
      ...
      .subscribe(
        ...
        (error) => {
          log.debug('Failed to fetch plugin catalog data', { error: error instanceof Error ? error.message : String(error) });
