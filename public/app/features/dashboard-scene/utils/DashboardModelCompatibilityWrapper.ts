...
import { AnnotationQuery, DashboardCursorSync, dateTimeFormat, DateTimeInput, EventBusSrv, logError } from '@grafana/data';
...
export class DashboardModelCompatibilityWrapper {
  ...
  public removePanel(panel: PanelModelCompatibilityWrapper) {
    ...
    if (!vizPanel) {
      logError('Trying to remove a panel that was not found in scene', panel);
      return;
    }
