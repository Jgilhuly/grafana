...
import { sceneLogger } from 'app/core/utils/logger';
  ...
  async buildDebugDashboard() {
    ...
    if (snapshot) {
      try {
        ...
        sceneLogger.error('Error creating scene from snapshot', { panelTitle: panel.state.title }, ex instanceof Error ? ex : undefined);
      }
    }
