...
const log = createStructuredLogger('public.app.features.dashboardScene.inspect.HelpWizard.SupportSnapshotService');
  ...
  async buildDebugDashboard() {
    ...
    if (snapshot) {
      try {
        ...
        log.debug('Error creating scene from snapshot', { error: ex instanceof Error ? ex.message : String(ex) });
      }
    }
