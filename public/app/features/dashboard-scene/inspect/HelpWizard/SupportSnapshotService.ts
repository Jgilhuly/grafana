...
const logger = createMonitoringLogger('dashboard-scene.support-snapshot');
  ...
  async buildDebugDashboard() {
    ...
    if (snapshot) {
      try {
        ...
        logger.logError(ex instanceof Error ? ex : new Error(String(ex)), { location: 'buildDebugDashboard' });
      }
    }
