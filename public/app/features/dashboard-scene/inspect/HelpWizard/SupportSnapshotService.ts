...
import { dashboardSceneLogger } from 'app/core/utils/logging';
  ...
  async buildDebugDashboard() {
    ...
    if (snapshot) {
      try {
        ...
        dashboardSceneLogger.logError(ex instanceof Error ? ex : new Error(String(ex)), {
          context: 'SupportSnapshotService.updateSnapshot',
        });
      }
    }
