import { logError, logInfo, store } from '@grafana/data';
...
export function writePerformanceGroupEnd(): void {
  if (isPerformanceLoggingEnabled()) {
    logInfo('Performance group ended');
  }
}
...
export function createPerformanceMark(name: string, timestamp?: number): void {
  try {
    ...
    logError(`❌ Failed to create performance mark: ${name}`, { timestamp, error });
  }
}
...
export function createPerformanceMeasure(name: string, startMark: string, endMark?: string): void {
  try {
    ...
    logError(`❌ Failed to create performance measure: ${name}`, { startMark, endMark, error });
  }
}
