...
const logger = createStructuredLogger('features.canvas.frame');
    ...
    switch (action) {
      ...
      case LayerActionID.Duplicate:
        if (element.item.id === 'frame') {
          logger.logWarning('Cannot duplicate frame elements yet', { action, elementId: element.item.id });
          return;
        }
      ...
      default:
        logger.logDebug('Unhandled layer action', { action, elementId: element.item.id });
        return;
    }
  };
