...
const logger = createMonitoringLogger('features.canvas.frameRuntime');
    ...
    switch (action) {
      ...
      case LayerActionID.Duplicate:
        if (element.item.id === 'frame') {
          logger.logWarning('Can not duplicate frames (yet)', {
            action,
            elementId: element.item.id,
      ...
      default:
        logger.logDebug('Unhandled layer action', { action, elementId: element.item.id, elementName: element.options.name });
        return;
    }
  };
