...
import { datasourcesLogger } from './utils/logging';
...
export const getDataSourceByIdOrUid = async (idOrUid: string) => {
  ...
  try {
    ...
    datasourcesLogger.logDebug('Failed to lookup data source using UID', { idOrUid });
  }
  ...
  try {
    ...
    datasourcesLogger.logDebug('Failed to lookup data source using ID', { idOrUid });
  }
