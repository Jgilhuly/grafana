...
import { exploreLogger } from 'app/features/explore/utils/logging';
...
function criticalPathForTrace(trace: Trace) {
  ...
  if (rootSpanId) {
    ...
    try {
      ...
      exploreLogger.logError(error instanceof Error ? error : new Error(String(error)), { context: 'computing critical path for trace' });
    }
  }
  return criticalPath;
}
