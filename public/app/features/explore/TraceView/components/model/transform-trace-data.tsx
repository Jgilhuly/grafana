...
// @ts-ignore
import { logWarning, TraceKeyValuePair } from '@grafana/data';
...
export default function transformTraceData(data: TraceResponse | undefined): Trace | null {
  ...
  for (let i = 0; i < max; i++) {
    ...
    if (idCount != null) {
      logWarning(`Dupe spanID, ${idCount + 1} x ${spanID}`, span, spanMap.get(spanID));
      if (_isEqual(span, spanMap.get(spanID))) {
        logWarning('\t two spans with same ID have `isEqual(...) === true`');
      }
