...
import { createFeatureLogger } from 'app/core/utils/logging';
...
const logger = createFeatureLogger('EchoSrv');
...
export async function initEchoSrv() {
  ...
  try {
    ...
    logger.error('Error initializing Performance backend', error);
  }

  try {
    await initFaroBackend();
  } catch (error) {
    logger.error('Error initializing Faro backend', error);
  }

  try {
    await initGoogleAnalyticsBackend();
  } catch (error) {
    logger.error('Error initializing GoogleAnalytics backend', error);
  }

  try {
    await initGoogleAnalaytics4Backend();
  } catch (error) {
    logger.error('Error initializing GoogleAnalaytics4 backend', error);
  }

  try {
    await initRudderstackBackend();
  } catch (error) {
    logger.error('Error initializing Rudderstack backend', error);
  }

  try {
    await initAzureAppInsightsBackend();
  } catch (error) {
    logger.error('Error initializing AzureAppInsights backend', error);
  }

  try {
    await initConsoleBackend();
  } catch (error) {
    logger.error('Error initializing Console backend', error);
  }
}
