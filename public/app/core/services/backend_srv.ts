...
import { BackendSrv as BackendService, BackendSrvRequest, config, FetchError, FetchResponse, logError, logDebug } from '@grafana/runtime';
    ...
    return {
      next: (response) => {
        ...
        process()
          ...
          .catch((e) => {
            logDebug('Request catch', { requestId, error: String(e) });
            observer.error(e);
          }); // from abort
      },
      error: (e) => {
