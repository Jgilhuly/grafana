...
export class LivePanel extends PureComponent<Props, State> {
  ...
  streamObserver: PartialObserver<LiveChannelEvent> = {
    next: (event: LiveChannelEvent) => {
      if (isLiveChannelStatusEvent(event)) {
        ...
        logger.logDebug('Ignoring unhandled event type');
      }
    },
  };
  ...
  async loadChannel() {
    ...
    if (!isValidLiveChannelAddress(addr)) {
      logger.logDebug('Invalid channel address');
      this.unsubscribe();
      this.setState({
        addr: undefined,
      });
    ...
    if (isEqual(addr, this.state.addr)) {
      logger.logDebug('Channel already loaded', { scope: this.state.addr?.scope, namespace: this.state.addr?.namespace });
      return;
    }

    const live = getGrafanaLiveSrv();
    if (!live) {
      logger.logDebug('Live service not available');
      this.unsubscribe();
      this.setState({
        addr: undefined,
      });
