...
export class LivePanel extends PureComponent<Props, State> {
  ...
  streamObserver: PartialObserver<LiveChannelEvent> = {
    next: (event: LiveChannelEvent) => {
      if (isLiveChannelStatusEvent(event)) {
        ...
        pluginLogger.logDebug('Ignoring live channel event', { eventType: String(event.type) });
      }
    },
  };
  ...
  async loadChannel() {
    ...
    if (!isValidLiveChannelAddress(addr)) {
      pluginLogger.logWarning('Invalid live channel address', { addr: JSON.stringify(addr) });
      this.unsubscribe();
      this.setState({
        addr: undefined,
      });
    ...
    if (isEqual(addr, this.state.addr)) {
      // Same channel already loaded
      return;
    }

    const live = getGrafanaLiveSrv();
    if (!live) {
      pluginLogger.logWarning('Grafana Live service not available', { addr: JSON.stringify(addr) });
      this.unsubscribe();
      this.setState({
        addr: undefined,
      });
    ...
    pluginLogger.logDebug('Loading live channel', { addr: JSON.stringify(addr) });
