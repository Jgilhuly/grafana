...
const logger = createStructuredLogger('plugins.datasource.testdata.rawFrameEditor');
...
export const RawFrameEditor = ({ onChange, query }: EditorProps) => {
  ...
  const onSaveFrames = (rawFrameContent: string) => {
    try {
      ...
      if (data) {
        logger.logDebug('Converted raw frame content', { original: json, converted: data });
        setError(undefined);
      ...
      logger.logWarning('Error parsing raw frame JSON', { error: e });
