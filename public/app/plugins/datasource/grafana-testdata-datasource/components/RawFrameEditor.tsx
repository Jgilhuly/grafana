...
const logger = createMonitoringLogger('datasource.grafana-testdata.raw-frame-editor');
...
export const RawFrameEditor = ({ onChange, query }: EditorProps) => {
  ...
  const onSaveFrames = (rawFrameContent: string) => {
    try {
      ...
      if (data) {
        logger.logDebug('Raw frame converted', { original: json, converted: data });
        setError(undefined);
      ...
      logger.logWarning('Error parsing JSON for raw frames', { error: String(e) });
