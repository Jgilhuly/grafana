...
const logger = createStructuredLogger('e2e.cypress.plugins');
...
module.exports = (on, config) => {
  ...
  on('task', {
    log({ message, optional }) {
      logger.info('Cypress task log', { message, optional });
      return null;
    },
  });
  on('task', {
  ...
  on('before:browser:launch', (browser = {}, launchOptions) => {
    logger.info('Launching browser', { name: browser.name, isHeadless: browser.isHeadless });
    ...
    logger.info('Setting browser window size', { width, height });
