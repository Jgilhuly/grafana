...
<TimeRangeInput
  value={timeRange}
  onChange={(range) => console.info(JSON.stringify({ level: 'info', message: 'Time range changed', context: { range } }))}
  onChangeTimeZone={(tz) =>
    console.info(JSON.stringify({ level: 'info', message: 'Time zone changed', context: { tz } }))
  }
/>;
```
