...
import { logError, logInfo } from '../../../../scripts/utils/structuredLogger';
...
if (!API_TOKEN) {
  logError('Error: CROWDIN_PERSONAL_TOKEN environment variable is not set');
  process.exit(1);
}

const PROJECT_ID = process.env.CROWDIN_PROJECT_ID ? parseInt(process.env.CROWDIN_PROJECT_ID, 10) : undefined;
if (!PROJECT_ID) {
  logError('Error: CROWDIN_PROJECT_ID environment variable is not set');
  process.exit(1);
}
...
async function getLanguages(projectId: number) {
  try {
    ...
    logError('Failed to fetch languages: ', error.message);
...
async function getFileIds(projectId: number) {
  try {
    ...
    logError('Failed to fetch file IDs: ', error.message);
    if (error.response && error.response.data) {
      logError('Error details: ', JSON.stringify(error.response.data, null, 2));
    }
    process.exit(1);
...
async function getWorkflowStepId(projectId: number) {
  try {
    ...
    logError('Failed to fetch workflow step ID: ', error.message);
    if (error.response && error.response.data) {
      logError('Error details: ', JSON.stringify(error.response.data, null, 2));
    }
    process.exit(1);
...
async function createTask(projectId: number, title: string, languageId: string, fileIds: number[], workflowStepId: number) {
  try {
    ...
    logError('Failed to create Crowdin task: ', error.message);
    if (error.response && error.response.data) {
      logError('Error details: ', JSON.stringify(error.response.data, null, 2));
    }
    process.exit(1);
